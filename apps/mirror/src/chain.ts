...
//! Same as the publisher's reader.
...
export async function fetchCommitteeData(rpcUrl: string, typeScript: ScriptJson, fetchFn: typeof fetch = fetch): Promise<PublisherSetData> {
  ...
  const body = (await response.json()) as { result?: { objects: { output_data: Hex }[] }; error?: unknown };
  ...
  return decodePublisherSetData(body.result.objects[0]!.output_data);
}
